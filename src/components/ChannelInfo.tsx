...
export default function ChannelInfo({ channel, visible, streamInfo }: ChannelInfoProps) {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-20 info-slide sm:p-0">
      <div className="relative h-auto sm:h-[172px] overflow-hidden bg-gradient-to-t from-black via-black/95 to-[#111214]/92 px-6 sm:px-[43px] pb-8 pt-5 shadow-[0_-28px_70px_rgba(0,0,0,0.92)] backdrop-blur-sm">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/18 to-transparent" />

        <div className="grid h-full grid-cols-1 sm:grid-cols-[minmax(300px,1.2fr)_1px_minmax(200px,0.75fr)_1px_minmax(200px,0.75fr)_auto] items-end gap-4 sm:gap-5">
          <section className="min-w-0">
            <div className="mb-3 sm:mb-4 flex items-center gap-3 sm:gap-4">
              <div className="font-mono text-xl sm:text-[22px] font-black text-white">{String(channel.number).padStart(3, '0')}</div>
              <div className="flex h-12 w-12 sm:h-[60px] sm:w-[60px] items-center justify-center rounded-[12px] sm:rounded-[14px] bg-[#202124] text-xl sm:text-2xl shadow-xl shadow-black/35">
                {channel.logo.startsWith('http') || channel.logo.startsWith('/') ? (
                  <img src={channel.logo} alt={channel.name} className="max-h-8 sm:max-h-10 max-w-10 sm:max-w-12 object-contain" />
                ) : (
                  channel.logo
                )}
              </div>
            ...
            <div className="mb-0.5 sm:mb-1 text-[8px] sm:text-[9px] font-black uppercase tracking-[0.22em] text-white/30">Now</div>
          ...
          <div className="hidden sm:block h-[43px] w-px bg-white/15" />

          <section className="hidden sm:block min-w-0 pb-[9px]">
          ...
          <section className="flex items-center sm:items-start gap-8 self-center sm:self-start sm:pt-[18px]">
            {channel.stream && (
